...
function MediaItemTake:get_info_string(param)
	...
	local retval, info = reaper.GetSetMediaItemTakeInfo_String(self.pointer, param, "", false)
	if retval then
		return info
	end
end
...
function MediaItemTake:set_info_string(param, value)
	local retval, _ = reaper.GetSetMediaItemTakeInfo_String(self.pointer, param, value, true)
	if not retval then
...
function MediaItemTake:set_pcm_source(
	source --[[PCMSource]]
)
