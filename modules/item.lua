...
local media_item_take = require("modules.take")
...
function MediaItem:__tostring()
	return string.format("<MediaItem GUID=%s>", self:GUID())
end
...
-- @return number
function MediaItem:get_info_value(
	param --[[string]]
)
...
-- @return string
function MediaItem:get_info_string(
	param --[[string]]
)
	--if MediaItemInfoValue[param] == nil then
	--    error('Invalid param. Please use constants.MediaItemInfoValue')
...
-- @param string : Accepted param values: constants.MediaItemInfoValue
-- @value
function MediaItem:set_info_value(
	param --[[string]],
	value --[[any]]
)
	--if MediaItemInfoValue[param] == nil then
...
-- @param string : Accepted param values: constants.MediaItemInfoString
-- @value string
function MediaItem:set_info_string(
	param --[[string]],
	value --[[string]]
)
	--if MediaItemInfoValue[param] == nil then
	--    error('Invalid param. Please use constants.MediaItemInfoValue')
	--end
	local retval, info = r.GetSetMediaItemInfo_String(self.pointer, param, value, true)
end

function MediaItem:set_length(
	length --[[number]],
	refreshUI --[[boolean]]
)
...
function MediaItem:set_position(
	position --[[number]],
	refreshUI --[[boolean]]
)
	refreshUI = refreshUI or true
...
function MediaItem:get_state_chunk(
...
function MediaItem:set_state_chunk(
	state --[[string]],
	is_undo --[[boolean - optional]]
)
	is_undo = is_undo or false
