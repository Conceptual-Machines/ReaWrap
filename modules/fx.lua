...
function TrackFX:__tostring()
	return string.format("<FX idx=%s, name=%s>", self.idx, self:get_name())
end
...
function TrackFX:get_param_name(param_idx)
	local retval, buf = r.TrackFX_GetParamName(self.track.pointer, self.idx, param_idx, "")
	if retval then
		return buf
	end
end
...
function TrackFX:get_param_range(param_idx)
	local retval, minval, maxval, midval = reaper.TrackFX_GetParamEx(self.track.pointer, self.idx, param_idx)
	if retval then
...
function TrackFX:get_param_step_size(param_idx)
	local retval, step, small, large, is_toggle =
		r.TrackFX_GetParameterStepSizes(self.track.pointer, self.idx, param_idx)
	if retval then
...
function TrackFX:get_pin_mappings(io, pin)
	...
	local bitmask = r.TrackFX_GetPinMappings(self.track.pointer, self.idx, io_map[io], pin)
...
function InputPin:__tostring()
	return string.format("<InputPin name=%s idx=%s>", self:get_name(), self.idx)
end
...
function OutputPin:__tostring()
	return string.format("<OutputPin name=%s idx=%s>", self:get_name(), self.idx)
end
...
function FXParam:__tostring()
	return string.format("<FXParam name=%s idx=%s>", self:get_name(), self.idx)
end
