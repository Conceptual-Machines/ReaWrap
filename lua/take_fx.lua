...
function TakeFX:format_param_value_normalized(param, value, buf)
  local ret_val, buf =
    r.TakeFX_FormatParamValueNormalized(self.take.pointer, self.pointer, param, value, buf)
  if ret_val then
    return buf
  else
...
function TakeFX:get_param_ex(param)
  local ret_val, min_val, max_val, mid_val =
    r.TakeFX_GetParamEx(self.take.pointer, self.pointer, param)
  if ret_val then
...
function TakeFX:set_pin_mappings(is_output, pin, low32bits, hi32bits)
  return r.TakeFX_SetPinMappings(
    self.take.pointer,
    self.pointer,
    is_output,
    pin,
