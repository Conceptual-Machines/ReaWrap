...
function PCM:calculate_normalization(normalize_to, normalize_target, normalize_start, normalize_end)
  return r.CalculateNormalization(
    self.pointer,
    normalize_to,
    normalize_target,
    normalize_start,
...
function PCM:enum_media_source_cues(index)
  local ret_val, time, end_time, is_region, name, is_chapter =
    r.CF_EnumMediaSourceCues(self.pointer, index)
  if ret_val then
...
--- @return number
function PCM:get_media_source_samples(
  dest_buf,
  dest_buf_offset,
  num_frames,
  num_chans,
