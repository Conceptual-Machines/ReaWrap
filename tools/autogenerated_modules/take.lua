-- @description Provide implementation for Take functions.
-- @author NomadMonad
-- @license MIT
...
-- @param take . The pointer to Reaper MediaItem_Take*
-- @return Take table.
...
-- @return number
function Take:get_peaks(peakrate, starttime, numchannels, numsamplesperchannel, want_extra_type, buf)
...
-- @return number
function Take:get_info_value(parm_name)
...
function Take:get_set_info_string(parm_name, string_need_big, set_new_value)
    local ret_val, string_need_big = r.GetSetMediaItemTakeInfo_String(self.pointer, parm_name, string_need_big, set_new_value)
    if ret_val then
...
function Take:get_take_envelope(env_idx)
...
-- @return Envelope table
function Take:get_take_envelope_by_name(env_name)
    local Envelope = require('envelope')
...
function Take:get_take_stretch_marker(idx)
    local ret_val, pos, number = r.GetTakeStretchMarker(self.pointer, idx)
    if ret_val then
        return pos, number
    else
        return nil
...
-- item.
-- @return notecnt number
-- @return ccevtcnt number
-- @return textsyxevtcnt number
...
--- Midi Delete Text Sysex Evt.
-- Delete a MIDI text or sysex event.
-- @param textsyxevt_idx number
-- @return boolean
function Take:midi_delete_text_sysex_evt(textsyxevt_idx)
...
-- @return chanmsg number
-- @return chan number
-- @return msg2 number
-- @return msg3 number
function Take:midi_get_cc(cc_idx)
    local ret_val, selected, muted, ppqpos, chanmsg, chan, msg2, msg3 = r.MIDI_GetCC(self.pointer, cc_idx)
    if ret_val then
...
function Take:midi_get_cc_shape(cc_idx)
    local ret_val, shape, beztension = r.MIDI_GetCCShape(self.pointer, cc_idx)
    if ret_val then
...
function Take:midi_get_evt(evt_idx)
    local ret_val, selected, muted, ppqpos, msg = r.MIDI_GetEvt(self.pointer, evt_idx)
    if ret_val then
...
function Take:midi_get_note(note_idx)
    local ret_val, selected, muted, startppqpos, endppqpos, chan, pitch, vel = r.MIDI_GetNote(self.pointer, note_idx)
    if ret_val then
...
--- Midi Get Ppq Pos From Proj Time.
-- Returns the MIDI tick (ppq) position corresponding to a specific project time in
-- seconds.
...
-- @return integer type
-- @return string msg
function Take:midi_get_text_sysex_evt(textsyxevt_idx, boolean, boolean, number, integer, string)
    local boolean = boolean or nil
    local number = number or nil
    local integer = integer or nil
    local string = string or nil
    local ret_val, boolean, boolean, number, integer, string = r.MIDI_GetTextSysexEvt(self.pointer, textsyxevt_idx, boolean, boolean, number, integer, string)
    if ret_val then
...
-- @param muted boolean
-- @param ppqpos number
-- @param chanmsg number
-- @param chan number
-- @param msg2 number
-- @param msg3 number
-- @return boolean
function Take:midi_insert_cc(selected, muted, ppqpos, chanmsg, chan, msg2, msg3)
...
-- @return boolean
function Take:midi_insert_evt(selected, muted, ppqpos, bytestr)
...
-- @return boolean
function Take:midi_insert_note(selected, muted, startppqpos, endppqpos, chan, pitch, vel, boolean)
    local boolean = boolean or nil
    return r.MIDI_InsertNote(self.pointer, selected, muted, startppqpos, endppqpos, chan, pitch, vel, boolean)
end

    
--- Midi Insert Text Sysex Evt.
...
-- @param bytestr string
-- @return boolean
function Take:midi_insert_text_sysex_evt(selected, muted, ppqpos, type, bytestr)
    return r.MIDI_InsertTextSysexEvt(self.pointer, selected, muted, ppqpos, type, bytestr)
end

    
--- Midi Refresh Editors.
...
-- @param boolean noSortIn Optional
-- @return boolean
function Take:midi_set_cc(cc_idx, boolean, boolean, number, integer, integer, integer, integer, boolean)
    local boolean = boolean or nil
    local number = number or nil
    local integer = integer or nil
...
-- @param boolean noSortIn Optional
-- @return boolean
function Take:midi_set_cc_shape(cc_idx, shape, beztension, boolean)
    local boolean = boolean or nil
...
-- @param boolean noSortIn Optional
-- @return boolean
function Take:midi_set_evt(evt_idx, boolean, boolean, number, string, boolean)
    local boolean = boolean or nil
    local number = number or nil
    local string = string or nil
...
-- @param boolean noSortIn Optional
-- @return boolean
function Take:midi_set_note(note_idx, boolean, boolean, number, number, integer, integer, integer, boolean)
    local boolean = boolean or nil
    local number = number or nil
    local integer = integer or nil
...
-- @param boolean noSortIn Optional
-- @return boolean
function Take:midi_set_text_sysex_evt(textsyxevt_idx, boolean, boolean, number, integer, string, boolean)
    local boolean = boolean or nil
    local number = number or nil
    local integer = integer or nil
    local string = string or nil
...
-- @return boolean
function Take:set_info_value(parm_name, newvalue)
...
-- @return number
function Take:set_take_marker(idx, name_in, number, integer)
    local number = number or nil
    local integer = integer or nil
...
-- @param number srcposIn Optional
-- @return number
function Take:set_take_stretch_marker(idx, pos, number)
    local number = number or nil
...
function Take:analyze_take_loudness(analyze_true_peak)
    local ret_val, lufs_integrated, range, true_peak, true_peak_pos, short_term_max, momentary_max = r.NF_AnalyzeTakeLoudness(self.pointer, analyze_true_peak)
    if ret_val then
...
function Take:analyze_take_loudness2(analyze_true_peak)
    local ret_val, lufs_integrated, range, true_peak, true_peak_pos, short_term_max, momentary_max, short_term_max_pos, momentary_max_pos = r.NF_AnalyzeTakeLoudness2(self.pointer, analyze_true_peak)
    if ret_val then
...
-- @return boolean
function Take:get_set_source_state2(state, setnewvalue)
