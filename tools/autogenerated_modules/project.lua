...
-- @param color number
-- @return number
function Project:add_project_marker(is_rgn, pos, rgnend, name, want_idx, color)
...
-- @param nudge_flag number
-- @param nudge_what number
-- @param nudge_units number
-- @param value number
...
--- Count Tcpfx Parms. Wraps CountTCPFXParms.
...
--- Get Tcpfx Parm. Wraps GetTCPFXParm.
...
function Project:get_tempo_time_sig_marker(pt_idx)
    local ret_val, time_pos, measure_pos, beat_pos, bpm, time_sig_num, time_sig_denom, lineartempo = r.GetTempoTimeSigMarker(self.pointer, pt_idx)
    if ret_val then
        return time_pos, measure_pos, beat_pos, bpm, time_sig_num, time_sig_denom, lineartempo
    else
        return nil
    end
end
...
--- Has Track Midi Programs Ex. Wraps HasTrackMIDIProgramsEx.
...
-- @return boolean
function Project:set_tempo_time_sig_marker(pt_idx, time_pos, measure_pos, beat_pos, bpm, time_sig_num, time_sig_denom, lineartempo)
    return r.SetTempoTimeSigMarker(self.pointer, pt_idx, time_pos, measure_pos, beat_pos, bpm, time_sig_num, time_sig_denom, lineartempo)
end
...
-- @return number
function Project:beats_to_time(t_pos, integer)
    local integer = integer or nil
    return r.TimeMap2_beatsToTime(self.pointer, t_pos, integer)
end
...
function Project:time_to_beats(t_pos)
    local ret_val, integer, integer, number, integer = r.TimeMap2_timeToBeats(self.pointer, t_pos)
    if ret_val then
...
-- @param t_pos number
-- @return number
function Project:time_to_qn(t_pos)
    return r.TimeMap2_timeToQN(self.pointer, t_pos)
end
...
-- @param t_pos number
-- @return number
function Project:time_to_qn_abs(t_pos)
    return r.TimeMap_timeToQN_abs(self.pointer, t_pos)
end
...
--- Validate Ptr2. Wraps ValidatePtr2.
