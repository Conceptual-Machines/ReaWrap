...
function Reaper:dock_is_child_of_dock(hwnd)
    return r.DockIsChildOfDock(hwnd)
end
...
function Reaper:enum_installed_fx(index)
    return r.EnumInstalledFX(index)
end
...
function Reaper:enum_pitch_shift_modes(mode)
    return r.EnumPitchShiftModes(mode)
end
...
function Reaper:enum_project_markers(idx)
    return r.EnumProjectMarkers(idx)
end
...
function Reaper:enum_projects(idx)
    local result = r.EnumProjects(idx)
...
function Reaper:enum_track_midi_program_names(track, program_number, program_name)
    return r.EnumTrackMIDIProgramNames(track, program_number, program_name)
end
...
function Reaper:get_config_var_string(name)
    return r.get_config_var_string(name)
end
...
function Reaper:get_action_shortcut_desc(section, cmd_id, shortcut_idx)
    return r.GetActionShortcutDesc(section, cmd_id, shortcut_idx)
end
...
function Reaper:get_audio_device_info(attribute)
    return r.GetAudioDeviceInfo(attribute)
end
...
function Reaper:get_midi_input_name(dev, nameout)
    return r.GetMIDIInputName(dev, nameout)
end
...
function Reaper:get_midi_output_name(dev, nameout)
    return r.GetMIDIOutputName(dev, nameout)
end
...
function Reaper:get_thing_from_point(screen_x, screen_y)
    local result = r.GetThingFromPoint(screen_x, screen_y)
    return media_track.MediaTrack:new(result)
end
...
function Reaper:get_touched_or_focused_fx(mode)
    return r.GetTouchedOrFocusedFX(mode)
end
...
function Reaper:get_track_from_point(screen_x, screen_y)
    local result = r.GetTrackFromPoint(screen_x, screen_y)
    return media_track.MediaTrack:new(result)
end
...
function Reaper:get_user_file_name_for_read(filename_need4096, title, defext)
    return r.GetUserFileNameForRead(filename_need4096, title, defext)
end
...
function Reaper:get_user_inputs(title, num_inputs, captions_csv, retvals_csv)
    return r.GetUserInputs(title, num_inputs, captions_csv, retvals_csv)
end
...
function Reaper:kbd_enumerate_actions(section, idx)
    return r.kbd_enumerateActions(section, idx)
end
...
-- @param command integer.
-- @param flag integer.
function Reaper:main_on_command_ex(command, flag)
...
--- Main Update Loop Info.
-- @param ignoremask integer.
function Reaper:main_update_loop_info(ignoremask)
    return r.Main_UpdateLoopInfo(ignoremask)
end

    
--- Master Get Play Rate At Time.
-- @param time_s number.
-- @return number
function Reaper:master_get_play_rate_at_time(time_s)
...
-- @return number
function Reaper:master_normalize_play_rate(playrate, is_normalized)
    return r.Master_NormalizePlayRate(playrate, is_normalized)
end

    
--- Master Normalize Tempo.
-- Convert the tempo to/from a value between 0 and 1, representing bpm in the range
-- of 40-296 bpm.
-- @param bpm number.
-- @param is_normalized boolean.
-- @return number
function Reaper:master_normalize_tempo(bpm, is_normalized)
...
function Reaper:menu_get_hash(menu_name, flag)
    return r.Menu_GetHash(menu_name, flag)
end
...
-- extend folder
-- @param before_track_idx integer.
...
function Reaper:theme_layout_get_layout(section, idx)
    return r.ThemeLayout_GetLayout(section, idx)
end
...
function Reaper:theme_layout_get_parameter(wp)
    return r.ThemeLayout_GetParameter(wp)
end
...
-- @param time number.
-- @return number
function Reaper:time_map_get_divided_bpm_at_time(time)
...
-- @param qn number.
-- @return number
function Reaper:time_map_qn_to_time(qn)
...
-- @param tpos number.
-- @return number
function Reaper:time_map_time_to_qn(tpos)
