-- @description ImGui: Provide implementation for ImGui functions.
-- @author NomadMonad
-- @license MIT
...
function ImGui:accept_drag_drop_payload(ctx, type, payload, integer)
    ...
    local retval, payload = r.ImGui_AcceptDragDropPayload(ctx, type, payload, integer)
    if retval then
        return payload
    else
        return nil
...
--- Accept Drag Drop Payload Files.
...
function ImGui:accept_drag_drop_payload_files(ctx, count, integer)
    local integer = integer or nil
    local retval, count = r.ImGui_AcceptDragDropPayloadFiles(ctx, count, integer)
    if retval then
        return count
    else
        return nil
...
function ImGui:accept_drag_drop_payload_rgb(ctx, rgb, integer)
    local integer = integer or nil
    local retval, rgb = r.ImGui_AcceptDragDropPayloadRGB(ctx, rgb, integer)
    if retval then
        return rgb
    else
        return nil
...
function ImGui:accept_drag_drop_payload_rgba(ctx, rgba, integer)
    local integer = integer or nil
    local retval, rgba = r.ImGui_AcceptDragDropPayloadRGBA(ctx, rgba, integer)
    if retval then
        return rgba
    else
        return nil
...
function ImGui:begin(ctx, name, boolean, integer)
    ...
    local integer = integer or nil
    local retval, boolean = r.ImGui_Begin(ctx, name, boolean, integer)
    if retval then
        return boolean
    else
        return nil
...
function ImGui:begin_popup_modal(ctx, name, boolean, integer)
    local boolean = boolean or nil
    local integer = integer or nil
    local retval, boolean = r.ImGui_BeginPopupModal(ctx, name, boolean, integer)
    if retval then
        return boolean
    else
        return nil
...
function ImGui:begin_tab_item(ctx, label, boolean, integer)
    local boolean = boolean or nil
    local integer = integer or nil
    local retval, boolean = r.ImGui_BeginTabItem(ctx, label, boolean, integer)
    if retval then
        return boolean
    else
        return nil
...
function ImGui:checkbox(ctx, label, v)
    local retval, v = r.ImGui_Checkbox(ctx, label, v)
    if retval then
        return v
    else
        return nil
...
function ImGui:checkbox_flags(ctx, label, flags, flags_value)
    local retval, flags = r.ImGui_CheckboxFlags(ctx, label, flags, flags_value)
    if retval then
        return flags
    else
        return nil
...
function ImGui:collapsing_header(ctx, label, boolean, integer)
    local boolean = boolean or nil
    local integer = integer or nil
    local retval, boolean = r.ImGui_CollapsingHeader(ctx, label, boolean, integer)
    if retval then
        return boolean
    else
        return nil
...
-- @param integer flagsIn optional
-- @return col_rgb integer
function ImGui:color_edit3(ctx, label, col_rgb, integer)
    local integer = integer or nil
    local retval, col_rgb = r.ImGui_ColorEdit3(ctx, label, col_rgb, integer)
    if retval then
        return col_rgb
    else
        return nil
...
-- @param col_rgba integer
-- @param integer flagsIn optional
-- @return col_rgba integer
function ImGui:color_edit4(ctx, label, col_rgba, integer)
    local integer = integer or nil
    local retval, col_rgba = r.ImGui_ColorEdit4(ctx, label, col_rgba, integer)
    if retval then
        return col_rgba
    else
        return nil
...
function ImGui:color_picker3(ctx, label, col_rgb, integer)
    local integer = integer or nil
    local retval, col_rgb = r.ImGui_ColorPicker3(ctx, label, col_rgb, integer)
    if retval then
        return col_rgb
    else
        return nil
...
function ImGui:color_picker4(ctx, label, col_rgba, integer, integer)
    local integer = integer or nil
    local retval, col_rgba = r.ImGui_ColorPicker4(ctx, label, col_rgba, integer, integer)
    if retval then
        return col_rgba
    else
        return nil
...
-- @return current_item integer
function ImGui:combo(ctx, label, current_item, items, integer)
    local integer = integer or nil
    local retval, current_item = r.ImGui_Combo(ctx, label, current_item, items, integer)
    if retval then
        return current_item
    else
        return nil
...
--- Config Flags No Saved Settings.
...
--- Config Var Debug Begin Return Value Loop.
...
--- Config Var Debug Begin Return Value Once.
...
--- Config Var Docking Transparent Payload.
...
--- Config Var Drag Click To Input Text.
...
--- Config Var Hover Flags For Tooltip Mouse.
...
--- Config Var Hover Flags For Tooltip Nav.
-- Default flags when using IsItemHovered(HoveredFlags_ForTooltip) or
...
--- Config Var Hover Stationary Delay.
...
--- Config Var Input Text Cursor Blink.
...
--- Config Var Input Text Enter Keep Active.
...
--- Config Var Input Trickle Event Queue.
...
--- Config Var Mouse Double Click Max Dist.
...
--- Config Var Mouse Double Click Time.
...
--- Config Var Mouse Drag Threshold.
...
--- Config Var Viewports No Decoration.
...
--- Config Var Windows Move From Title Bar Only.
...
--- Config Var Windows Resize From Edges.
...
-- @param ctx ImGui_Context
-- @param _idx integer
function ImGui:debug_flash_style_color(ctx, _idx)
    return r.ImGui_DebugFlashStyleColor(ctx, _idx)
end
...
function ImGui:drag_double(ctx, label, v, number, number, number, string, integer)
    ...
    local string = string or nil
    local integer = integer or nil
    local retval, v = r.ImGui_DragDouble(ctx, label, v, number, number, number, string, integer)
    if retval then
        return v
    else
        return nil
...
function ImGui:drag_double2(ctx, label, v1, v2, number, number, number, string, integer)
    ...
    local string = string or nil
    local integer = integer or nil
    local retval, v1, v2 = r.ImGui_DragDouble2(ctx, label, v1, v2, number, number, number, string, integer)
    if retval then
        return v1, v2
    else
        return nil
...
function ImGui:drag_double3(ctx, label, v1, v2, v3, number, number, number, string, integer)
    ...
    local string = string or nil
    local integer = integer or nil
    local retval, v1, v2, v3 = r.ImGui_DragDouble3(ctx, label, v1, v2, v3, number, number, number, string, integer)
    if retval then
        return v1, v2, v3
    else
        return nil
...
function ImGui:drag_double4(ctx, label, v1, v2, v3, v4, number, number, number, string, integer)
    ...
    local string = string or nil
    local integer = integer or nil
    local retval, v1, v2, v3, v4 = r.ImGui_DragDouble4(ctx, label, v1, v2, v3, v4, number, number, number, string, integer)
    if retval then
        return v1, v2, v3, v4
    else
        return nil
...
function ImGui:drag_float_range2(ctx, label, v_current_min, v_current_max, number, number, number, string, string, integer)
    ...
    local string = string or nil
    local integer = integer or nil
    local retval, v_current_min, v_current_max = r.ImGui_DragFloatRange2(ctx, label, v_current_min, v_current_max, number, number, number, string, string, integer)
    if retval then
...
function ImGui:drag_int(ctx, label, v, number, integer, integer, string, integer)
    ...
    local integer = integer or nil
    local string = string or nil
    local retval, v = r.ImGui_DragInt(ctx, label, v, number, integer, integer, string, integer)
    if retval then
        return v
    else
        return nil
...
function ImGui:drag_int2(ctx, label, v1, v2, number, integer, integer, string, integer)
    ...
    local integer = integer or nil
    local string = string or nil
    local retval, v1, v2 = r.ImGui_DragInt2(ctx, label, v1, v2, number, integer, integer, string, integer)
    if retval then
        return v1, v2
    else
        return nil
...
function ImGui:drag_int3(ctx, label, v1, v2, v3, number, integer, integer, string, integer)
    ...
    local integer = integer or nil
    local string = string or nil
    local retval, v1, v2, v3 = r.ImGui_DragInt3(ctx, label, v1, v2, v3, number, integer, integer, string, integer)
    if retval then
        return v1, v2, v3
    else
        return nil
...
function ImGui:drag_int4(ctx, label, v1, v2, v3, v4, number, integer, integer, string, integer)
    ...
    local integer = integer or nil
    local string = string or nil
    local retval, v1, v2, v3, v4 = r.ImGui_DragInt4(ctx, label, v1, v2, v3, v4, number, integer, integer, string, integer)
    if retval then
        return v1, v2, v3, v4
    else
        return nil
...
-- @param string format_maxIn optional
-- @param integer flagsIn optional
-- @return v_current_min integer
-- @return v_current_max integer
function ImGui:drag_int_range2(ctx, label, v_current_min, v_current_max, number, integer, integer, string, string, integer)
    local number = number or nil
    local integer = integer or nil
    local string = string or nil
    local retval, v_current_min, v_current_max = r.ImGui_DragIntRange2(ctx, label, v_current_min, v_current_max, number, integer, integer, string, string, integer)
    if retval then
        return v_current_min, v_current_max
    else
        return nil
...
-- @param splitter ImGui_DrawListSplitter
-- @param channel__idx integer
function ImGui:draw_list_splitter_set_current_channel(splitter, channel__idx)
    return r.ImGui_DrawListSplitter_SetCurrentChannel(splitter, channel__idx)
end
...
-- @return integer
function ImGui:get_color(ctx, _idx, number)
    local number = number or nil
    return r.ImGui_GetColor(ctx, _idx, number)
end
...
--- Get Config Var.
-- @param ctx ImGui_Context
-- @param var__idx integer
-- @return number
function ImGui:get_config_var(ctx, var__idx)
    return r.ImGui_GetConfigVar(ctx, var__idx)
end
...
-- @return type string
-- @return payload string
-- @return is__preview boolean
-- @return is__delivery boolean
function ImGui:get_drag_drop_payload(ctx)
    local retval, type, payload, is__preview, is__delivery = r.ImGui_GetDragDropPayload(ctx)
    if retval then
        return type, payload, is__preview, is__delivery
    else
        return nil
    end
end
...
function ImGui:get_drag_drop_payload_file(ctx, index)
    local retval, filename = r.ImGui_GetDragDropPayloadFile(ctx, index)
    if retval then
        return filename
    else
        return nil
...
--- Get Input Queue Character.
-- Read from ImGui's character input queue. Call with increasing idx until false is
-- returned.
-- @param ctx ImGui_Context
-- @param _idx integer
-- @return unicode_char integer
function ImGui:get_input_queue_character(ctx, _idx)
    local retval, unicode_char = r.ImGui_GetInputQueueCharacter(ctx, _idx)
    if retval then
        return unicode_char
    else
        return nil
...
-- @param _idx integer
-- @return integer
function ImGui:get_style_color(ctx, _idx)
    return r.ImGui_GetStyleColor(ctx, _idx)
end
...
-- @return val1 number
-- @return val2 number
function ImGui:get_style_var(ctx, var__idx)
    return r.ImGui_GetStyleVar(ctx, var__idx)
end
...
-- @return imgui_version_num integer
...
function ImGui:input_double(ctx, label, v, number, number, string, integer)
    ...
    local string = string or nil
    local integer = integer or nil
    local retval, v = r.ImGui_InputDouble(ctx, label, v, number, number, string, integer)
    if retval then
        return v
    else
        return nil
...
function ImGui:input_double2(ctx, label, v1, v2, string, integer)
    local string = string or nil
    local integer = integer or nil
    local retval, v1, v2 = r.ImGui_InputDouble2(ctx, label, v1, v2, string, integer)
    if retval then
        return v1, v2
    else
        return nil
...
function ImGui:input_double3(ctx, label, v1, v2, v3, string, integer)
    local string = string or nil
    local integer = integer or nil
    local retval, v1, v2, v3 = r.ImGui_InputDouble3(ctx, label, v1, v2, v3, string, integer)
    if retval then
        return v1, v2, v3
    else
        return nil
...
function ImGui:input_double4(ctx, label, v1, v2, v3, v4, string, integer)
    local string = string or nil
    local integer = integer or nil
    local retval, v1, v2, v3, v4 = r.ImGui_InputDouble4(ctx, label, v1, v2, v3, v4, string, integer)
    if retval then
        return v1, v2, v3, v4
    else
        return nil
...
function ImGui:input_int(ctx, label, v, integer, integer, integer)
    local integer = integer or nil
    local retval, v = r.ImGui_InputInt(ctx, label, v, integer, integer, integer)
    if retval then
        return v
    else
        return nil
...
function ImGui:input_int2(ctx, label, v1, v2, integer)
    local integer = integer or nil
    local retval, v1, v2 = r.ImGui_InputInt2(ctx, label, v1, v2, integer)
    if retval then
        return v1, v2
    else
        return nil
...
function ImGui:input_int3(ctx, label, v1, v2, v3, integer)
    local integer = integer or nil
    local retval, v1, v2, v3 = r.ImGui_InputInt3(ctx, label, v1, v2, v3, integer)
    if retval then
        return v1, v2, v3
    else
        return nil
...
function ImGui:input_int4(ctx, label, v1, v2, v3, v4, integer)
    local integer = integer or nil
    local retval, v1, v2, v3, v4 = r.ImGui_InputInt4(ctx, label, v1, v2, v3, v4, integer)
    if retval then
        return v1, v2, v3, v4
    else
        return nil
...
function ImGui:input_text(ctx, label, buf, integer, callback_in)
    local integer = integer or nil
    local retval, buf = r.ImGui_InputText(ctx, label, buf, integer, callback_in)
    if retval then
        return buf
    else
        return nil
...
function ImGui:input_text_multiline(ctx, label, buf, number, number, integer, callback_in)
    local number = number or nil
    local integer = integer or nil
    local retval, buf = r.ImGui_InputTextMultiline(ctx, label, buf, number, number, integer, callback_in)
    if retval then
        return buf
    else
        return nil
...
function ImGui:input_text_with_hint(ctx, label, hint, buf, integer, callback_in)
    local integer = integer or nil
    local retval, buf = r.ImGui_InputTextWithHint(ctx, label, hint, buf, integer, callback_in)
    if retval then
        return buf
    else
        return nil
...
function ImGui:list_box(ctx, label, current_item, items, integer)
    local integer = integer or nil
    local retval, current_item = r.ImGui_ListBox(ctx, label, current_item, items, integer)
    if retval then
        return current_item
    else
        return nil
...
--- List Clipper Get Display Range.
-- @param clipper ImGui_ListClipper
-- @return display_start integer
-- @return display_end integer
...
function ImGui:menu_item(ctx, label, string, boolean, boolean)
    local string = string or nil
    local boolean = boolean or nil
    local retval, boolean = r.ImGui_MenuItem(ctx, label, string, boolean, boolean)
    if retval then
        return boolean
    else
        return nil
...
-- @param _idx integer
-- @param col_rgba integer
function ImGui:push_style_color(ctx, _idx, col_rgba)
    return r.ImGui_PushStyleColor(ctx, _idx, col_rgba)
end
...
function ImGui:push_style_var(ctx, var__idx, val1, number)
    local number = number or nil
    return r.ImGui_PushStyleVar(ctx, var__idx, val1, number)
end
...
function ImGui:radio_button_ex(ctx, label, v, v_button)
    local retval, v = r.ImGui_RadioButtonEx(ctx, label, v, v_button)
    if retval then
        return v
    else
        return nil
...
function ImGui:selectable(ctx, label, boolean, integer, number, number)
    ...
    local integer = integer or nil
    local number = number or nil
    local retval, boolean = r.ImGui_Selectable(ctx, label, boolean, integer, number, number)
    if retval then
        return boolean
    else
        return nil
...
-- @param var__idx integer
-- @param value number
function ImGui:set_config_var(ctx, var__idx, value)
    return r.ImGui_SetConfigVar(ctx, var__idx, value)
end
...
-- @param ctx ImGui_Context
-- @param is__open boolean
-- @param integer condIn optional
function ImGui:set_next_item_open(ctx, is__open, integer)
    local integer = integer or nil
    return r.ImGui_SetNextItemOpen(ctx, is__open, integer)
end
...
function ImGui:slider_angle(ctx, label, v_rad, number, number, string, integer)
    ...
    local string = string or nil
    local integer = integer or nil
    local retval, v_rad = r.ImGui_SliderAngle(ctx, label, v_rad, number, number, string, integer)
    if retval then
        return v_rad
    else
        return nil
...
function ImGui:slider_double(ctx, label, v, v_min, v_max, string, integer)
    local string = string or nil
    local integer = integer or nil
    local retval, v = r.ImGui_SliderDouble(ctx, label, v, v_min, v_max, string, integer)
    if retval then
        return v
    else
        return nil
...
function ImGui:slider_double2(ctx, label, v1, v2, v_min, v_max, string, integer)
    local string = string or nil
    local integer = integer or nil
    local retval, v1, v2 = r.ImGui_SliderDouble2(ctx, label, v1, v2, v_min, v_max, string, integer)
    if retval then
        return v1, v2
    else
        return nil
...
function ImGui:slider_double3(ctx, label, v1, v2, v3, v_min, v_max, string, integer)
    local string = string or nil
    local integer = integer or nil
    local retval, v1, v2, v3 = r.ImGui_SliderDouble3(ctx, label, v1, v2, v3, v_min, v_max, string, integer)
    if retval then
        return v1, v2, v3
    else
        return nil
...
function ImGui:slider_double4(ctx, label, v1, v2, v3, v4, v_min, v_max, string, integer)
    local string = string or nil
    local integer = integer or nil
    local retval, v1, v2, v3, v4 = r.ImGui_SliderDouble4(ctx, label, v1, v2, v3, v4, v_min, v_max, string, integer)
    if retval then
        return v1, v2, v3, v4
    else
        return nil
...
function ImGui:slider_int(ctx, label, v, v_min, v_max, string, integer)
    local string = string or nil
    local integer = integer or nil
    local retval, v = r.ImGui_SliderInt(ctx, label, v, v_min, v_max, string, integer)
    if retval then
        return v
    else
        return nil
...
function ImGui:slider_int2(ctx, label, v1, v2, v_min, v_max, string, integer)
    local string = string or nil
    local integer = integer or nil
    local retval, v1, v2 = r.ImGui_SliderInt2(ctx, label, v1, v2, v_min, v_max, string, integer)
    if retval then
        return v1, v2
    else
        return nil
...
function ImGui:slider_int3(ctx, label, v1, v2, v3, v_min, v_max, string, integer)
    local string = string or nil
    local integer = integer or nil
    local retval, v1, v2, v3 = r.ImGui_SliderInt3(ctx, label, v1, v2, v3, v_min, v_max, string, integer)
    if retval then
        return v1, v2, v3
    else
        return nil
...
function ImGui:slider_int4(ctx, label, v1, v2, v3, v4, v_min, v_max, string, integer)
    local string = string or nil
    local integer = integer or nil
    local retval, v1, v2, v3, v4 = r.ImGui_SliderInt4(ctx, label, v1, v2, v3, v4, v_min, v_max, string, integer)
    if retval then
        return v1, v2, v3, v4
    else
        return nil
...
--- Table Get Column Sort Specs.
-- Sorting specification for one column of a table. Call while incrementing 'id'
-- from 0 until false is returned.
-- @param ctx ImGui_Context
-- @param id integer
-- @return column_index integer
-- @return column_user_id integer
-- @return sort_direction integer
function ImGui:table_get_column_sort_specs(ctx, id)
    local retval, column_index, column_user_id, sort_direction = r.ImGui_TableGetColumnSortSpecs(ctx, id)
    if retval then
...
function ImGui:table_need_sort(ctx)
    local retval, has_specs = r.ImGui_TableNeedSort(ctx)
    if retval then
        return has_specs
    else
        return nil
...
function ImGui:v_slider_double(ctx, label, size_w, size_h, v, v_min, v_max, string, integer)
    local string = string or nil
    local integer = integer or nil
    local retval, v = r.ImGui_VSliderDouble(ctx, label, size_w, size_h, v, v_min, v_max, string, integer)
    if retval then
        return v
    else
        return nil
...
function ImGui:v_slider_int(ctx, label, size_w, size_h, v, v_min, v_max, string, integer)
    local string = string or nil
    local integer = integer or nil
    local retval, v = r.ImGui_VSliderInt(ctx, label, size_w, size_h, v, v_min, v_max, string, integer)
    if retval then
        return v
    else
        return nil
