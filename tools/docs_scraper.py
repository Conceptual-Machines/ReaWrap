            ...
            if len(parts) == 1:
                ...
                if parts[0] == 'optional':
                    ...
                    if parts == ['ImGui_Context', 'ctxImGui_Font', 'font']:
                        yield ReaType(name='ctx', lua_type='ImGui_Context',
                                      is_optional=is_optional)
                        yield ReaType(name='font', lua_type='ImGui_Font',
                                      is_optional=is_optional)

                    elif parts == ['string', 'labelreaper_array', 'values']:
                        yield ReaType(name='label', lua_type='string',
                                      is_optional=is_optional)
                        yield ReaType(name='values',
                                      lua_type='reaper_array',
                                      is_optional=is_optional)
                    elif parts == ['ImGui_DrawList',
                                   'draw_listreaper_array', 'points']:
                        yield ReaType(name='draw_list',
                                      lua_type='ImGui_DrawList',
                                      is_optional=is_optional)
                        yield ReaType(name='values',
                                      lua_type='reaper_array',
                                      is_optional=is_optional)
                    elif parts == ['ImGui_DrawList', 'draw_listImGui_Font',
                                   'font']:
                        yield ReaType(name='draw_list',
                                      lua_type='ImGui_DrawList',
                                      is_optional=is_optional)
                        yield ReaType(name='font',
                                      lua_type='ImGui_Font',
                                      is_optional=is_optional)
                    elif parts == ['string', 'filenameLICE_IBitmap',
                                   'bitmap']:
                        yield ReaType(name='filename',
                                      lua_type='string',
                                      is_optional=is_optional)
                        yield ReaType(name='bitmap',
                                      lua_type='LICE_IBitmap',
                                      is_optional=is_optional)

                    elif parts == ['boolean', 'isrgnWDL_FastString',
                                   'name']:
                        yield ReaType(name='isrgn',
                                      lua_type='boolean',
                                      is_optional=is_optional)
                        yield ReaType(name='name',
                                      lua_type='WDL_FastString',
                                      is_optional=is_optional)
                    elif parts == ['identifier', 'objWDL_FastString',
                                   'state']:
                        yield ReaType(name='identifier',
                                      lua_type='obj',
                                      is_optional=is_optional)
                        yield ReaType(name='state',
                                      lua_type='WDL_FastString',
                                      is_optional=is_optional)

                    elif parts == ['integer', 'takeidxWDL_FastString',
                                   'state']:
                        yield ReaType(name='takeidx',
                                      lua_type='integer',
                                      is_optional=is_optional)
                        yield ReaType(name='state',
                                      lua_type='WDL_FastString',
                                      is_optional=is_optional)
                    elif parts == ['MediaItem_Take', 'takeWDL_FastString',
                                   'state']:
                        yield ReaType(name='take',
                        ...
                        yield ReaType(name='take',
                                      lua_type='MediaItem_Take',
                                      is_optional=is_optional)
                        yield ReaType(name='type',
                                      lua_type='WDL_FastString',
                                      is_optional=is_optional)

            elif len(parts) == 4:
                if parts == ['MRP_Array', 'array1MRP_Array',
                             'array2MRP_Array', 'array3']:
                    for i in range(1, 4):
                        yield ReaType(name=f'array_{i}',
                                      lua_type='MRP_Array',
                                      is_optional=is_optional)
            yield ReaType(name=stringcase.snakecase(safe_name(name)), lua_type=lua_type,
                          is_optional=is_optional)
...
class ReaWrapFunc:
            ...
            if self.instance_var:
                if reawrap:
                    return [
                        ...
                        for a in self.rea_func.args[1:] if a is not None
                    ]
                else:
                ...
                return [
                    stringcase.snakecase(a.name)
                    for a in self.rea_func.args if a is not None
                ]
        else:
            return []
        ...
        for v in self.rea_func.ret_vals:
            if v is not None:
                if v.name in arg_names:
                    ret_vals.append(
                        ReaType(name=f'{v.name}_', lua_type=v.lua_type)
                    )
                else:
                    ret_vals.append(v)
        if as_string:
    ...
    def get_lua_func_call(self):
        ...
        if self.get_reawrap_ret_vals() == self.get_reascript_ret_vals():
            ...
            return func_call_template.render(rea=reascript_vals, reawrap=reawrap_vals, func=func_string)
    ...
    def generate_code(self):
        ...
        if self.rea_func.args:
            if self.instance_var:
                args = '\n'.join([f'-- @{a.name} {a.lua_type}' for a in self.rea_func.args[1:] if a is not None])
            else:
                args = '\n'.join([f'-- @{a.name} {a.lua_type}' for a in self.rea_func.args if a is not None])
...
def parse_lua_code(lua_code, docs):
    try:
        ...
        if any(x for x in (func_sig, parsed_ret_vals, parsed_args, docs)):
            return ReaFunc(signature=func_sig, ret_vals=parsed_ret_vals,
                           args=parsed_args, docs=docs)
    except Exception as e:
        pass
...
def main():
    ...
    if os.listdir(folder):
        for f in os.listdir(folder):
            os.remove(folder / f)
    functions = sorted(list(iter_funcs()),
                       key=lambda x: x.get_reawrap_signature())
